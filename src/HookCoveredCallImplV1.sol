    ...
    PermissionConstants
{
    ...
    function initialize(address protocol, address nftContract, address hookVaultFactory, address preApprovedMarketplace)
        public
        initializer
    {
    ...
    function mintWithVault(
        ...
        require(allowedUnderlyingAddress == vault.assetAddress(assetId), "mWV-token not allowed");
        ...
        vault.imposeEntitlement(address(this), expirationTime, assetId, signature.v, signature.r, signature.s);

        return _mintOptionWithVault(writer, vault, assetId, strikePrice, expirationTime);
    }

    /// @dev See {IHookCoveredCall-mintWithEntitledVault}.
    function mintWithEntitledVault(address vaultAddress, uint32 assetId, uint128 strikePrice, uint32 expirationTime)
        external
        nonReentrant
        whenNotPaused
        returns (uint256)
    {
        ...
        require(allowedUnderlyingAddress == vault.assetAddress(assetId), "mWEV-token not allowed");
        require(vault.getHoldsAsset(assetId), "mWEV-asset must be in vault");
        (bool active, address operator) = vault.getCurrentEntitlementOperator(assetId);
        require(active && operator == address(this), "mWEV-call contract not operator");

        require(expirationTime == vault.entitlementExpiration(assetId), "mWEV-entitlement expiration different");
        require(
            _allowedVaultImplementation(vaultAddress, allowedUnderlyingAddress, assetId),
    ...
    function mintWithErc721(address tokenAddress, uint256 tokenId, uint128 strikePrice, uint32 expirationTime)
        external
        nonReentrant
        whenNotPaused
        returns (uint256)
    {
        ...
        require(allowedUnderlyingAddress == tokenAddress, "mWE7-token not on allowlist");

        require(
            msg.sender == tokenOwner || IERC721(tokenAddress).isApprovedForAll(tokenOwner, msg.sender)
        ...
        IHookERC721Vault vault = _erc721VaultFactory.findOrCreateVault(tokenAddress, tokenId);

        uint32 assetId = 0;
        if (
            address(vault)
                == Create2.computeAddress(
                    BeaconSalts.multiVaultSalt(tokenAddress), BeaconSalts.ByteCodeHash, address(_erc721VaultFactory)
                )
        ) {
        ...
        uint256 optionId = _mintOptionWithVault(tokenOwner, IHookVault(vault), assetId, strikePrice, expirationTime);
        ...
        IERC721(tokenAddress).safeTransferFrom(
            tokenOwner, address(vault), tokenId, abi.encode(tokenOwner, address(this), expirationTime)
        );

        // make sure that the vault actually has the asset.
        require(IERC721(tokenAddress).ownerOf(tokenId) == address(vault), "mWE7-asset not in vault");
    ...
    function _mintOptionWithVault(
        ...
        require(expirationTime > block.timestamp + minimumOptionDuration, "_mOWV-expires sooner than min duration");
        ...
        if (prevOptionId != 0) {
            require(optionParams[prevOptionId].settled, "_mOWV-previous option must be settled");
        }
        ...
        emit CallCreated(writer, address(vault), assetId, newOptionId, strikePrice, expirationTime);
    ...
    modifier biddingEnabled(uint256 optionId) {
        ...
        require((call.expiration - settlementAuctionStartOffset) <= block.timestamp, "bE-bidding starts on last day");
    ...
    function _allowedVaultImplementation(address vaultAddress, address underlyingAddress, uint32 assetId)
        internal
        view
        returns (bool)
    {
        ...
        if (
            vaultAddress
                == Create2.computeAddress(
                    BeaconSalts.multiVaultSalt(underlyingAddress), BeaconSalts.ByteCodeHash, address(_erc721VaultFactory)
                )
        ) {
            return true;
        }
    ...
    function bid(uint256 optionId) external payable nonReentrant biddingEnabled(optionId) {
        ...
        require(
            bidAmt >= call.bid + ((call.bid * minBidIncrementBips) / 10000), "b-must overbid by minBidIncrementBips"
        );
    ...
    function reclaimAsset(uint256 optionId, bool returnNft) external nonReentrant {
        CallOption storage call = optionParams[optionId];
        ...
        if (call.highBidder != address(0)) {
            ...
            IHookVault(call.vaultAddress).setBeneficialOwner(call.assetId, call.writer);
        }
    ...
    function burnExpiredOption(uint256 optionId) external nonReentrant whenNotPaused {
        CallOption storage call = optionParams[optionId];
    ...
    modifier onlyMarketController() {
        require(_protocol.hasRole(MARKET_CONF, msg.sender), "caller needs MARKET_CONF");
        _;
    }
    ...
    function setMinOptionDuration(uint256 newMinDuration) public onlyMarketController {
    ...
    function setBidIncrement(uint256 newBidIncrement) public onlyMarketController {
    ...
    function setSettlementAuctionStartOffset(uint256 newSettlementStartOffset) public onlyMarketController {
    ...
    function getVaultAddress(uint256 optionId) public view override returns (address) {
        return optionParams[optionId].vaultAddress;
    }

    /// @dev see {IHookCoveredCall-getOptionIdForAsset}
    function getOptionIdForAsset(address vault, uint32 assetId) external view returns (uint256) {
    ...
    function getStrikePrice(uint256 optionId) public view override returns (uint256) {
        return optionParams[optionId].strike;
    }

    /// @dev see {IHookCoveredCall-getExpiration}.
    function getExpiration(uint256 optionId) public view override returns (uint256) {
