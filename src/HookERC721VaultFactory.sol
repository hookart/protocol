...
contract HookERC721VaultFactory is IHookERC721VaultFactory, PermissionConstants {
    ...
    mapping(address => mapping(uint256 => IHookERC721Vault)) public override getVault;
    ...
    constructor(address hookProtocolAddress, address beaconAddress, address multiBeaconAddress) {
        ...
        require(Address.isContract(multiBeaconAddress), "multi beacon address must be a contract");
    ...
    function makeMultiVault(address nftAddress) external returns (IHookERC721Vault) {
        require(
        ...
        require(getMultiVault[nftAddress] == IHookERC721Vault(address(0)), "makeMultiVault-vault cannot already exist");

        IInitializeableBeacon bp = IInitializeableBeacon(
            Create2.deploy(0, BeaconSalts.multiVaultSalt(nftAddress), type(HookBeaconProxy).creationCode)
        );

        bp.initializeBeacon(
            _multiBeacon,
            /// This is the ABI encoded initializer on the IHookERC721Vault.sol
            abi.encodeWithSignature("initialize(address,address)", nftAddress, _hookProtocol)
        );
    ...
    function makeSoloVault(address nftAddress, uint256 tokenId) public override returns (IHookERC721Vault) {
        require(getVault[nftAddress][tokenId] == IHookERC721Vault(address(0)), "makeVault-a vault cannot already exist");

        IInitializeableBeacon bp = IInitializeableBeacon(
            Create2.deploy(0, BeaconSalts.soloVaultSalt(nftAddress, tokenId), type(HookBeaconProxy).creationCode)
        );

        bp.initializeBeacon(
            _beacon,
            /// This is the ABI encoded initializer on the IHookERC721MultiVault.sol
            abi.encodeWithSignature("initialize(address,uint256,address)", nftAddress, tokenId, _hookProtocol)
        );
        IHookERC721Vault vault = IHookERC721Vault(address(bp));
    ...
    function findOrCreateVault(address nftAddress, uint256 tokenId) external returns (IHookERC721Vault) {
