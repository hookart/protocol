...
library VaultAuthenticator {
    function isHookERC721Vault(address vaultFactory, address underlyingAddress, address vaultAddress, uint32 assetId) public view returns (bool) {
    if (
      vaultAddress ==
  ...
  function isHookERC20Vault(address vaultFactory, address tokenAddress, address vaultAddress) public view returns (bool) {
    if (
      vaultAddress ==
      Create2.computeAddress(
