...
contract HookERC721VaultImplV1 is HookERC721MultiVaultImplV1 {
    ...
    ///-constructor
    function initialize(address nftContract, uint256 tokenId, address hookAddress) public {
        _tokenId = tokenId;
    ...
    function withdrawalAsset(uint32 assetId) public override assetIdIsZero(assetId) {
    ...
    function onERC721Received(
        ...
        if (msg.sender == address(_nftContract) && tokenId == _tokenId) {
            ...
            if (data.length > 0) {
                ...
                // will throw.
                (address _beneficialOwner, address entitledOperator, uint32 expirationTime) =
                ...
                _registerEntitlement(ASSET_ID, entitledOperator, expirationTime, assets[ASSET_ID].beneficialOwner);
            }
            emit AssetReceived(this.getBeneficialOwner(uint32(ASSET_ID)), operator, msg.sender, ASSET_ID);
        } else {
            ...
            require(
                !_hookProtocol.getCollectionConfig(address(_nftContract), keccak256("vault.airdropsProhibited")),
    ...
    function execTransaction(address to, bytes memory data) external payable virtual returns (bool) {
        ...
        require(to != address(_nftContract), "execTransaction-cannot send transactions to the NFT contract itself");

        // block transactions to the vault to mitigate reentrancy vulnerabilities
        require(to != address(this), "execTransaction-cannot call the vault contract");

        require(
            !_hookProtocol.getCollectionConfig(address(_nftContract), keccak256("vault.execTransactionDisabled")),
    ...
    function setBeneficialOwner(uint32 assetId, address newBeneficialOwner) public override assetIdIsZero(assetId) {
    ...
    function _assetTokenId(uint32 assetId) internal view override assetIdIsZero(assetId) returns (uint256) {
        return _tokenId;
    }
}
