...
contract HookERC721MultiVaultImplV1 is IHookERC721Vault, EIP712, Initializable, ReentrancyGuard {
    /// ----------------  STORAGE ---------------- ///
    ...
    ///-constructor
    function initialize(address nftContract, address hookAddress) public initializer {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual returns (bool) {
        return interfaceId == type(IHookERC721Vault).interfaceId || interfaceId == type(IERC165).interfaceId;
    }
    ...
    function onERC721Received(
        ...
        require(tokenId <= type(uint32).max, "onERC721Received-tokenId is out of range");
        ...
        if (msg.sender == address(_nftContract)) {
            ...
            if (data.length > 0) {
                ...
                if (data.length == 3 * 32) {
                    ...
                    // will throw.
                    (address beneficialOwner, address entitledOperator, uint32 expirationTime) =
                    ...
                    _registerEntitlement(uint32(tokenId), entitledOperator, expirationTime, beneficialOwner);
                } else {
                    ...
                    (address beneficialOwner, address entitledOperator, uint32 expirationTime, address approvedOperator)
            ...
            emit AssetReceived(this.getBeneficialOwner(uint32(tokenId)), operator, msg.sender, uint32(tokenId));
        } else {
            ...
            require(
                _hookProtocol.getCollectionConfig(address(_nftContract), keccak256("vault.multiAirdropsAllowed")),
    ...
    function flashLoan(uint32 assetId, address receiverAddress, bytes calldata params) external override nonReentrant {
        IERC721FlashLoanReceiver receiver = IERC721FlashLoanReceiver(receiverAddress);
        require(receiverAddress != address(0), "flashLoan-zero address");
        require(_assetOwner(assetId) == address(this), "flashLoan-asset not in vault");
        require(msg.sender == assets[assetId].beneficialOwner, "flashLoan-not called by the asset owner");

        require(
            !_hookProtocol.getCollectionConfig(address(_nftContract), keccak256("vault.flashLoanDisabled")),
        ...
        _nftContract.safeTransferFrom(address(this), receiverAddress, _assetTokenId(assetId));
        ...
        require(
            receiver.executeOperation(address(_nftContract), _assetTokenId(assetId), msg.sender, address(this), params),
        ...
        _nftContract.transferFrom(receiverAddress, address(this), _assetTokenId(assetId));
        ...
        // prematurely
        require(startState == keccak256(abi.encode(assets[assetId])), "flashLoan-entitlement state cannot be modified");

        // (7) emit an event to record the flashloan
        emit AssetFlashLoaned(assets[assetId].beneficialOwner, assetId, receiverAddress);
    }
    ...
    function clearEntitlementAndDistribute(uint32 assetId, address receiver) external nonReentrant {
        ...
        IERC721(_nftContract).safeTransferFrom(address(this), receiver, _assetTokenId(assetId));
    ...
    function _registerEntitlement(uint32 assetId, address operator, uint32 expiry, address beneficialOwner) internal {
        ...
        assets[assetId] = Asset({operator: operator, expiry: expiry, beneficialOwner: beneficialOwner});
