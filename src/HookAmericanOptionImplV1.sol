    ...
    PermissionConstants
{
    ...
    function mintWithVault(
        ...
        require(collateralAssetAddress == vault.assetAddress(assetId), "mWV-token not allowed");
        require(vault.getHoldsAsset(assetId), "mWV-asset not in vault");
        require(
            _allowedVaultImplementation(vaultAddress, collateralAssetAddress, assetId),
        ...
        return _mintOptionWithVault(writer, vault, assetId, expirationTime, vaultValidatorParams);
    }
    ...
    function mintWithEntitledVault(
        ...
        require(collateralAssetAddress == vault.assetAddress(assetId), "mWEV-token not allowed");
    ...
    function _allowedVaultImplementation(address vaultAddress, address underlyingAddress, uint32 assetId)
        ...
        returns (bool)
    {
        return VaultAuthenticator.isHookERC20Vault(_erc20VaultFactory, underlyingAddress, vaultAddress)
            || VaultAuthenticator.isHookERC721Vault(_erc721VaultFactory, underlyingAddress, vaultAddress, assetId);
    }

    // ----- END OF OPTION FUNCTIONS ---------//

    function exercisePut(uint256 optionId, address exerciseAssetVaultAddress, uint32 assetId) external nonReentrant {
        ...
        require(
            vaultValidator.validate(exerciseAssetVaultAddress, assetId, put.exercisableAssetVaultParameter),
            "exerciseAsset invalid"
        );
        ...
        IHookVault(put.considerationAssetVaultAddress).clearEntitlementAndDistribute(
            put.considerationAssetVaultId, optionOwner
        );
    ...
    function getStrikePrice(uint256 optionId) external view override returns (uint256) {
        return 0;
    }
}
