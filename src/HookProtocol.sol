...
contract HookProtocol is PermissionConstants, AccessControl, IHookProtocol, Pausable {
    ...
    constructor(
        ...
        require(allowlister != address(0), "allowlister address cannot be set to the zero address");
        require(pauser != address(0), "pauser address cannot be set to the zero address");
        require(vaultUpgrader != address(0), "admin address cannot be set to the zero address");
        require(callUpgrader != address(0), "callUpgrader address cannot be set to the zero address");
        require(marketConf != address(0), "marketConf address cannot be set to the zero address");
        require(collectionConf != address(0), "collectionConf address cannot be set to the zero address");
    ...
    function setCollectionConfig(address collectionAddress, bytes32 config, bool value)
        external
        onlyRole(COLLECTION_CONF)
    {
    ...
    modifier callUpgraderOnly() {
        require(hasRole(CALL_UPGRADER, msg.sender), "Caller is not a call upgrader");
        _;
    }

    modifier vaultUpgraderOnly() {
        require(hasRole(VAULT_UPGRADER, msg.sender), "Caller is not a vault upgrader");
        _;
    }
    ...
    function setCoveredCallFactory(address coveredCallFactoryContract) external callUpgraderOnly {
        require(
            Address.isContract(coveredCallFactoryContract), "setCoveredCallFactory: implementation is not a contract"
        );
    ...
    function setVaultFactory(address vaultFactoryContract) external vaultUpgraderOnly {
        require(Address.isContract(vaultFactoryContract), "setVaultFactory: implementation is not a contract");
