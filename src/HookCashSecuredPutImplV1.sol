...
contract HookCashSecuredPutImplV1 is
  IHookCoveredCall,
  ...
  PermissionConstants
{
  ...
  /// @notice The metadata for each cash-secured put stored within the protocol
  ...
  /// @param assetId the asset id of the cash within the vault
  /// @param vaultAddress the address of the vault holding the cash securing the put
  /// @param strike The strike price to exercise the put option
  ...
  struct PutOption {
    ...
    uint128 strike;
    address exercisableAssetAddress;
    uint256 exercisableAssetIdStart;
    uint256 exercisableAssetIdEnd;
    bool settled;
  }
  ...
  mapping(uint256 => PutOption) public optionParams;
