...
contract HookCoveredCallMintTests is HookProtocolTest {
    ...
    function testCannotMintOptionInvalidExpiration() public {
        ...
        vm.expectRevert("mint -- _expirationTime must be more than one day in the future time");
        calls.mint(
            address(token),
            underlyingTokenId,
            1000,
...
contract HookCoveredCallSettleTests is HookProtocolTest {
    ...
    function testSettleOptionReturnNft() public {
        ...
        address vaultAddress = vaultFactory.getVault(address(token), underlyingTokenId);
        vm.expectCall(
            vaultAddress,
            abi.encodeWithSignature("withdrawalAsset()")
        );
...
contract HookCoveredCallReclaimTests is HookProtocolTest {
    ...
    function testCannotReclaimAssetAsNonCallWriter() public {
        ...
        vm.expectRevert("reclaimAsset -- asset can only be reclaimed by the writer");
        calls.reclaimAsset(optionTokenId, true);
    }
    ...
    function testCannotReclaimWithActiveBid() public {
        ...
        vm.startPrank(writer);
        vm.expectRevert("reclaimAsset -- cannot reclaim a sold asset if the option is not writer-owned.");
        calls.reclaimAsset(optionTokenId, true);
    }
    ...
    function testCannotReclaimBeforeExpiration() public {
        ...
        vm.expectRevert("reclaimAsset -- the option must expired unless writer-owned");
        calls.reclaimAsset(optionTokenId, true);
    }
}
