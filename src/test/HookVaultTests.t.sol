...
contract HookVaultTestsBase is HookProtocolTest {
    ...
    function makeEntitlementAndSignature(uint256 ownerPkey, address operator, address vaultAddress, uint32 _expiry)
        internal
        returns (Entitlements.Entitlement memory, Signatures.Signature memory)
    {
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(ownerPkey, _getEIP712Hash(structHash));

        Signatures.Signature memory sig =
            Signatures.Signature({signatureType: Signatures.SignatureType.EIP712, v: v, r: r, s: s});
...
contract HookVaultTestFlash is HookVaultTestsBase {
    function testBasicFlashLoan() public {
        ...
        token.safeTransferFrom(writer, vaultAddress, tokenId, abi.encode(writer, mockContract, expiration));
        ...
        assertTrue(token.ownerOf(tokenId) == vaultAddress, "good flashloan should work");
    }
    ...
    function testFlashLoanFailsIfDisabled() public {
        ...
        IERC721FlashLoanReceiver flashLoan = new FlashLoanSuccess();
        vm.prank(admin);
        protocol.setCollectionConfig(address(token), keccak256("vault.flashLoanDisabled"), true);
        vm.prank(writer);
...
contract HookVaultTestEntitlement is HookVaultTestsBase {
    function testImposeEntitlmentOnTransferIn() public {
        ...
        HookERC721VaultImplV1 vaultImpl = HookERC721VaultImplV1(vaultAddress);
        assertTrue(vaultImpl.getHoldsAsset(0), "the token should be owned by the vault");
        assertTrue(vaultImpl.getBeneficialOwner(0) == writer, "writer should be the beneficial owner");
        assertTrue(vaultImpl.hasActiveEntitlement(0), "there should be an active entitlement");
    }
    ...
    function testImposeEntitlementAfterInitialTransfer() public {
        ...
        (Entitlements.Entitlement memory entitlement, Signatures.Signature memory sig) =
            makeEntitlementAndSignature(writerpkey, mockContract, vaultAddress, expiration);
        ...
        vaultImpl.imposeEntitlement(
            entitlement.operator, uint32(entitlement.expiry), uint32(entitlement.assetId), sig.v, sig.r, sig.s
        );
        ...
        vm.expectRevert("withdrawalAsset-the asset cannot be withdrawn with an active entitlement");
        vm.prank(writer);
    ...
    function testEntitlementGoesAwayAfterExpiration() public {
        ...
        assertTrue(!vaultImpl.hasActiveEntitlement(0), "there should not be any active entitlements");

        vm.prank(writer);
        vaultImpl.withdrawalAsset(0);
        assertTrue(!vaultImpl.getHoldsAsset(0), "the token should not be owned by the vault");

        assertTrue(token.ownerOf(tokenId) == writer, "token should be owned by the writer");
    }
    ...
    function testNewEntitlementPossibleAferExpiredEntitlement() public {
        ...
        (Entitlements.Entitlement memory entitlement2, Signatures.Signature memory sig2) =
            makeEntitlementAndSignature(writerpkey, mockContract, vaultAddress, expiration2);
        vaultImpl.imposeEntitlement(
            entitlement2.operator, uint32(entitlement2.expiry), uint32(entitlement2.assetId), sig2.v, sig2.r, sig2.s
        );
        assertTrue(vaultImpl.hasActiveEntitlement(0), "there should be a new active entitlement");
    }
    ...
    function testOnlyOneEntitlementAllowed() public {
        ...
        (Entitlements.Entitlement memory entitlement2, Signatures.Signature memory sig2) =
            makeEntitlementAndSignature(writerpkey, mockContract2, vaultAddress, expiration2);

        vm.prank(mockContract2);
        vm.expectRevert("_registerEntitlement-existing entitlement must be cleared before registering a new one");
    ...
    function testBeneficialOwnerCannotClearEntitlement() public {
        ...
        vm.prank(writer);
        vm.expectRevert("clearEntitlement-only the entitled address can clear the entitlement");
        vaultImpl.clearEntitlement(0);
...
contract HookVaultTestsDistribution is HookVaultTestsBase {
    ...
    function testClearAndDistributeReturnsNFT() public {
        ...
        assertTrue(token.ownerOf(tokenId) == writer, "Token should be returned to the owner");
    }
    ...
    function testAirdropsCanBeDisbled() public {
        ...
        vm.prank(admin);
        protocol.setCollectionConfig(address(token), keccak256("vault.airdropsProhibited"), true);

        TestERC721 token2 = new TestERC721();
        vm.expectRevert("onERC721Received-non-escrow asset returned when airdrops are disabled");
    ...
    function testAirdropsAllowedWhenEnabled() public {
        ...
        vm.prank(admin);
        protocol.setCollectionConfig(address(token), keccak256("vault.airdropsProhibited"), false);
    ...
    function testClearAndDistributeDoesNotReturnToWrongPerson() public {
        ...
        vm.expectRevert("clearEntitlementAndDistribute-Only the beneficial owner can receive the asset");
        vm.prank(mockContract);
