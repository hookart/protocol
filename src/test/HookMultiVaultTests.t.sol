...
contract HookMultiVaultTests is HookProtocolTest {
    ...
    function makeEntitlementAndSignature(
        ...
        uint256 tokenId,
        uint32 _expiry
    ) internal returns (Entitlements.Entitlement memory, Signatures.Signature memory) {
    ...
    function testImposeEntitlmentOnTransferIn() public {
        ...
        assertTrue(vaultImpl.getHoldsAsset(tokenId), "the token should be owned by the vault");
        assertTrue(vaultImpl.getBeneficialOwner(tokenId) == writer, "writer should be the beneficial owner");
        (bool active, address operator) = vaultImpl.getCurrentEntitlementOperator(tokenId);
        assertTrue(active, "there should be an active entitlement");
        assertTrue(operator == mockContract, "active entitlement is to correct person");
    }
    ...
    function testImposeEntitlementAfterInitialTransfer() public {
        ...
        (Entitlements.Entitlement memory entitlement, Signatures.Signature memory sig) =
            makeEntitlementAndSignature(writerpkey, mockContract, vaultAddress, tokenId, expiration);
    ...
    function testEntitlementGoesAwayAfterExpiration() public {
        ...
        vm.prank(writer);
        vaultImpl.withdrawalAsset(tokenId);
        assertTrue(!vaultImpl.getHoldsAsset(tokenId), "the token should not be owned by the vault");
    ...
    function testNewEntitlementPossibleAferExpiredEntitlement() public {
        ...
        (Entitlements.Entitlement memory entitlement2, Signatures.Signature memory sig2) =
            makeEntitlementAndSignature(writerpkey, mockContract, vaultAddress, tokenId, expiration2);
        vaultImpl.imposeEntitlement(
    ...
    function testOnlyOneEntitlementAllowed() public {
        ...
        (Entitlements.Entitlement memory entitlement2, Signatures.Signature memory sig2) =
            makeEntitlementAndSignature(writerpkey, mockContract2, vaultAddress, tokenId, expiration2);
    ...
    function testAirdropsAllowedWhenEnabled() public {
        ...
        vm.prank(admin);
        protocol.setCollectionConfig(address(token), keccak256("vault.multiAirdropsAllowed"), true);
