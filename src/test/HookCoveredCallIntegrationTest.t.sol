...
contract HookCoveredCallIntegrationTest is HookProtocolTest {
    ...
    function testRevertMintOptionMustBeOwnerOrOperator() public {
        ...
        calls.mintWithErc721(address(token), underlyingTokenId, 1000, uint32(block.timestamp + 3 days));
    }

    function testRevertMintOptionExpirationMustBeMoreThan1DayInTheFuture() public {
        vm.startPrank(address(writer));

        vm.expectRevert("_mOWV-expires sooner than min duration");
        calls.mintWithErc721(address(token), underlyingTokenId, 1000, uint32(block.timestamp + 30 minutes));
        vm.stopPrank();
    }
    ...
    function testSuccessfulAuctionAndSettlement() public {
        ...
        assertTrue(calls.currentBid(optionId) == 1050, "contract should update the current high bid for the option");
        assertTrue(calls.currentBidder(optionId) == bidder1, "bidder1 should be in the lead");
        assertTrue(bidder1.balance == 50, "bidder1 should have deposited money into escrow");
        ...
        // validate that bid is updated
        assertTrue(calls.currentBid(optionId) == 1100, "contract should update the current high bid for the option");
        assertTrue(calls.currentBidder(optionId) == bidder2, "bidder2 should be in the lead");
        assertTrue(bidder1.balance == 1100, "bidder1 should have their money back from escrow");
        ...
        assertTrue(writerEndBalance - writerStartBalance == 1000, "the writer gets the strike price");
        assertTrue(buyerEndBalance - buyerStartBalance == 100, "the call owner gets the spread");
    }
