...
contract HookCoveredCallIntegrationTest is HookProtocolTest {
  ...
  function testSuccessfulAuctionAndSettlement() public {
    ...
    function testSuccessfulAuctionAndSettlementAfterD() public {
    // create the call option
    vm.startPrank(address(writer));
    uint256 writerStartBalance = writer.balance;
    uint256 baseTime = block.timestamp;
    ...
    vm.startPrank(admin);
    HookCoveredCallImplV2 callImpl2 = new HookCoveredCallImplV2();
    callBeacon.upgradeTo(address(callImpl2));
    callBeacon.upgradeTo(address(callImpl));
    vm.stopPrank();
