...
contract HookProtocolTest is PTest, EIP712, PermissionConstants {
    ...
    event CallCreated(
        address writer, address vaultAddress, uint256 assetId, uint256 optionId, uint256 strikePrice, uint256 expiration
    );
    ...
    function setUpMintOption() public {
        ...
        optionTokenId = calls.mintWithErc721(address(token), underlyingTokenId, 1000, expiration);
    ...
    function makeSignature(uint256 tokenId, uint32 expiry, address _writer)
        internal
        returns (Signatures.Signature memory)
    {
        address va = address(vaultFactory.findOrCreateVault(address(token), tokenId));

        uint32 assetId = 0;
        if (
            va
                == Create2.computeAddress(
                    BeaconSalts.multiVaultSalt(address(token)), BeaconSalts.ByteCodeHash, address(vaultFactory)
                )
        ) {
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(writerpkey, _getEIP712Hash(structHash));
