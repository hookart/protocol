...
abstract contract HookInstrumentERC721 is ERC721Burnable {
    ...
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return operator == _preApprovedMarketplace || super.isApprovedForAll(owner, operator);
    }

    constructor(string memory instrumentType) ERC721(makeInstrumentName(instrumentType), "INST") {}

    function makeInstrumentName(string memory z) internal pure returns (string memory) {
    ...
    function getVaultAddress(uint256 optionId) public view virtual returns (address);
    ...
    function getStrikePrice(uint256 optionId) external view virtual returns (uint256);

    /// @notice getter for the options expiration. After this time the
    /// option is invalid
    function getExpiration(uint256 optionId) external view virtual returns (uint256);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (class == ERC_721) {
            ...
            return TokenURI.tokenURIERC721(
                tokenId, underlyingAddress, underlyingTokenId, instrumentExpiration, instrumentStrikePrice, transfers
            );
        }
    ...
    ///
    function _underlyingClass(uint256 optionId) internal view returns (bytes4) {
        if (ERC165Checker.supportsInterface(getVaultAddress(optionId), type(IHookERC721Vault).interfaceId)) {
            return ERC_721;
        } else {
